...
class Classifier4GT(PreTrainedModel):
    def __init__(self, model, n_labels, dropout=0.0, seed=0, cla_bias=True, feat_shrink=''):
        ...
        self.classifier = nn.Linear(self.hidden_dim, n_labels, bias=cla_bias)
    ...
    def forward(self,
        ...
        cls_embeddings = last_hidden_states[:, 1].view(B, N, self.hidden_dim)  # [B,N,D]
        node_embeddings = cls_embeddings[:, 0, :]  # [B,D]
        # outputs[0]=last hidden state
        cls_token_emb = self.dropout(node_embeddings)
